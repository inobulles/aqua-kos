	...
	#include "../external/discord/discord_rpc.h"
	...
	static void discord_device_handle(unsigned long long** result, const char* data) {
		...
		if      (strcmp(data, "rpc init") == 0) init_discord_rpc  ((unsigned long long) data);
		else if (strcmp(data, "rpc loop") == 0) loop_discord_rpc  (&discord_device_struct->discord_rpc);
		else if (strcmp(data, "rpc updt") == 0) update_discord_rpc(&discord_device_struct->discord_rpc);
		else if (strcmp(data, "rpc disp") == 0) dispose_discord_rpc();
