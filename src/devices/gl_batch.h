...
static void gl_batch_device_handle(unsigned long long** result, const char* data) {
	...
	if (batch_command[0] == 'p') { // draw
		...
		glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 0, object->positions);
		//~ glColorAttribPointer (1, 4, GL_FLOAT, GL_FALSE, 0, object->colours);
		...
		glDrawElements(GL_TRIANGLES, object->index_count, GL_UNSIGNED_SHORT, object->indices);
		...
		glDrawElements(GL_TRIANGLES, (GLsizei) object->index_count, GL_UNSIGNED_SHORT, object->indices);
		...
		if (old_vertex_count == 0) { // if nothing yet in batch, simply malloc
			...
			object->indices        = (GLshort*)                                   malloc(object-> index_count * sizeof(GLshort));
		}
			...
			object->indices        = (GLshort*)                                   realloc(object->indices,        object-> index_count * sizeof(GLshort));
		}
		...
		for (unsigned long long i = 0; i < object->index_count; i++) object->indices[i + old_index_count] = ((uint32_t*) model->index_pointer)[i];
	}
	
	else if (batch_command[0] == 'u') { // colour
		for (unsigned long long i = 0; i < object->vertex_count; i++) {
			object->colours[i].r = (float) ((unsigned long long) batch_command[2]) / UMAX;
			object->colours[i].g = (float) ((unsigned long long) batch_command[3]) / UMAX;
			object->colours[i].b = (float) ((unsigned long long) batch_command[4]) / UMAX;
			object->colours[i].a = (float) ((unsigned long long) batch_command[5]) / UMAX;
		}
	}
