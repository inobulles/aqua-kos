	...
	static void gl_batch_device_handle(unsigned long long** result, const char* data) {
		...
		if (batch_command[0] == 'p') { // draw
			...
			object->has_texture = (unsigned char) (batch_command[7] ? 1 : 0);
			...
			for (i = old_vertex_count; i < object->vertex_count; i++) { // loop through each vertex of added object
				...
				object->indices[i * 3    ] = (uint32_t) faces[absolute].pair1[0];
				object->indices[i * 3 + 1] = (uint32_t) faces[absolute].pair2[0];
				object->indices[i * 3 + 2] = (uint32_t) faces[absolute].pair3[0];
