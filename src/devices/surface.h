	...
	void surface_device_handle(unsigned long long** result, const char* data) {
		...
		if (command[0] == 'n') {
			kos_bda_implementation.temp_value = (unsigned long long) malloc(sizeof(surface_t));
			surface_new(kos_bda_implementation.temp_value, 0, 0, 2 * FLOAT_ONE, 2 * FLOAT_ONE);
		}
		
		else if (command[0] == 't') surface_set_texture(command[1], command[2]);
		else if (command[0] == 'l') surface_set_layer  (command[1], command[2]);
