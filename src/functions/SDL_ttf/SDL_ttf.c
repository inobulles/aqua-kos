...
//~ #include "SDL.h" /// MODIFIED FROM ORIGINAL SOURCE
#include <SDL2/SDL_endian.h> /// MODIFIED FROM ORIGINAL SOURCE
#include "SDL_ttf.h"
...
#include <freetype/fterrors.h>
    ...
    TTF_SetError(buffer);
#else
    TTF_SetError("%s", msg); /// MODIFIED FROM ORIGINAL SOURCE
...
TTF_Font* TTF_OpenFontIndex( const char *file, int ptsize, long index )
{
    ...
    if ( rw == NULL ) {
        TTF_SetError("%s", SDL_GetError()); /// MODIFIED FROM ORIGINAL SOURCE
        return NULL;
    }
