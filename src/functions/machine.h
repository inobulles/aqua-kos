	...
	#include <sys/wait.h>
	static unsigned long long kos_execute_machine(unsigned long long zvm, unsigned long long mid, unsigned long long _wait) {
		char ascii_width    [16];
	...
	unsigned long long kos_create_machine(unsigned long long zvm, unsigned long long __path, unsigned long long width, unsigned long long height, unsigned long long text_only) {
		GET_PATH(__path)
	...
	#include <signal.h>
	void kos_kill_machine(unsigned long long zvm, unsigned long long mid) {
		if (machines[mid].exists) kill(machines[mid].pid, SIGKILL);
		else printf("WARNING Machine (%lld) does not exist\n", mid);
		
	}
	
	void kos_give_machine_events(unsigned long long zvm, unsigned long long mid, events_t* events) {
		printf("TODO %s\n", __func__);
		
	}
	
	unsigned long long kos_current_machine(unsigned long long zvm) {
		return root_mid;
